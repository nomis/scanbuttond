// server.c: event propagation server
...
#include "scanbuttond/protocol.h"

#define DEF_BACKEND_FILENAME		"libscanbtnd-backend_meta.so"
...
void list_devices(scanner_t* devices)
{
	...
	while (dev != NULL) {
		syslog(LOG_INFO, "found scanner: vendor=\"%s\", product=\"%s\", sane_name=\"%s\"",
